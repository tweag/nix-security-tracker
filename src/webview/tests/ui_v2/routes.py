"""
Route definitions for UI v2 tests.
Mirrors the client-side routes from the frontend App.
To be kept in sync.
"""
# FIXME(@fricklerhandwerk): Read the routes from a single source of truth.

UI_V2_PREFIX = "/ui-v2"
HOME = f"{UI_V2_PREFIX}/"
USER_SETTINGS_SUBSCRIPTIONS = f"{UI_V2_PREFIX}/user/subscriptions"
USER_SETTINGS_TOKENS = f"{UI_V2_PREFIX}/user/tokens"
PACKAGE_SUBSCRIPTION = f"{UI_V2_PREFIX}/user/subscriptions/packages"
SUGGESTION_LIST = f"{UI_V2_PREFIX}/suggestions"
SUGGESTION_DETAIL = f"{UI_V2_PREFIX}/suggestions/by-id"
SUGGESTION_DETAIL_BY_CVE = f"{UI_V2_PREFIX}/suggestions/by-cve"
ISSUE_LIST = f"{UI_V2_PREFIX}/issues"
ISSUE_DETAIL = f"{UI_V2_PREFIX}/issues"
NOTIFICATIONS = f"{UI_V2_PREFIX}/notifications"
