...
The CSS is organized into multiple files that are loaded in `src/shared/templates/base.html`:

- **`reset.css`** - Resets browsers' default styles for consistency and building upon a clean slate
- **`font.css`** - Font face declarations for IBM Plex font family, the web font used on sec-tracker
- **`colors.css`** - Defines our limited color palette. You are expected to only use these colors and add new ones there only if strictly necessary. Colors should not be defined outside this. In other css files like `utility.css`, colors are referenced with `var(--color-name)`.
- **`cvss-tags.css`** - These defines the official colors for CVSS severities. These are not part of our design language, just there to mirror the official CVSS tags colors, and should not be used otherwise.
- **`utility.css`** - Utility classes for layout, typography, and various reusable UI components. This should contain all the classes you need to style your html templates. New classes should only be added there after due consideration, and never to introduce small ad hoc variations.
- **`page-layout.css`** - These define one-time use styles for the general page layout and one-time use elements.
- **`icons/style.css`** - This file is generated by the online icomoon tool. It introduces the classes to be used with the `<i>` tag for icons that are part of our custom icon font. Read more about icons below.

## Utility classes

Here are some of the utility classes. This is not an exhaustive list. Check `utility.css` and see existing templates for reference.
...
- `.dimmed` - Gray text color
- `.heading` - Box / section title
- `.page-title` - Only for the main title of a page
- `.prose` - To be applied on a container that has longer text content. It restores some expected styling (margins between paragraphs, bullets on item lists) that is removed by the `reset.css` for the rest of the UI.
...
- `.row` - Row flex box
- `.row-reverse` - Row flex box with elements from right to left
- `.column` - Column flex box
- `.inline-row` - Row flex box that is expected to be inlined (e.g. in a text)

Then use the optional following classes:

**Alignment:**

- `.centered` - Center items together (for row items, it means vertical centering, not that they'll be centered on the page)
- `.baseline` - align-items: baseline
- `.align-end` - align-items: flex-end
- `.spread` - Force items to be spread across the available space

**Spacing:**

- `.gap` - Normal gap between items
- `.gap-small` - Small gap between items
- `.dividers` - Use standard gap with dividers between lines

**Sizing:**

- `.full-width` - Force to use all the available space
- `.grow` - Make an item grow in the flex container
- `.wrap` - Allow items to wrap if necessary, this provides responsivity with no need for error-prone breakpoints

### Containers

- `.rounded-box` - Bordered container with padding and rounded corners
- `.toggler` + `.toggler-on` / `.toggler-off`- Big toggler element
...
- `.hl-red` - Light red background
- `.hl-green` - Light green background
- `.highlight` - Default yellow highlight background
- `.highlight-target` - Animation highlight for anchor targets, a brief highlight that fades away

### Buttons

- `.btn` - Base button style to be used on every standard button, in addition to one of the following
...
The icon font is generated using the online icomoon tool at https://icomoon.io/app/
...
Adding new styles shoud be a last resort:
